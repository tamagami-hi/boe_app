...
| BE-010 Activation and web/native auth routes | ACTIVE (BE-010a DONE; BE-010b next) | GATE-03, CON-009, BE-006, BE-008, BE-009 | Split into child packets. **BE-010a** native-session DB layer (`credentialRepository`, `authSessionRepository`: createNativeSession/lockByRefreshTokenHash/revokeAllForUser) proven on PG. Next: **010b** activation route (invite -> Argon2id credential + session + refresh + activate + audit), **010c** login/refresh-rotation/logout + Fastify auth guard; delete `security/auth.js` + legacy auth routes |
