...
| BE-008 Public consent/application/verification routes | ACTIVE (BE-008a + BE-008b-1 + BE-008b-2 DONE; BE-008c next) | GATE-03, BE-006, BE-007 | Split into child packets. **BE-008a** `GET /v1/public/consent-documents` DONE. **BE-008b-1** `src/crypto/` primitives + `CryptoContext` DONE. **BE-008b-2** `POST /v1/applications` submission (uniform 202, DB idempotency via advisory lock, atomic application+consents+token+delivery+outbox+audit) + 7 repository impls + `submitApplication` command + corrected `executeIdempotent`; integration 21/21 with a dedicated integration coverage gate over repositories/routes/domain DONE. Next: **BE-008c** verify-email + delete `website/services/onboardingService.js` (first backend JS deletion); **BE-008b-3** cooldown resend + cross-match + race savepoint. `publicRoutes.js` -> BE-013 |
| BE-009 Password/token/session security core | BACKLOG | GATE-03, BE-007, CON-009 | Argon2id, HIBP, ES256, refresh rotation; delete `src/security/*.js` |
