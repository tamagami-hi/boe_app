...
| BE-009 Password/token/session security core | ACTIVE (BE-009a DONE; BE-009b next) | GATE-03, BE-007, CON-009 | Split into child packets. **BE-009a** `src/auth/passwordHasher.ts` (Argon2id hash/verify/dummy + PasswordInput) + pinned `argon2@0.44.0`/`jose@6.2.3`; **deleted `src/security/passwords.js` (82 -> 81)**. Next: **009b** HIBP breach check, **009c** ES256 access tokens (jose), **009d** refresh/CSRF rotation; then delete `security/{auth,tokens}.js` |
