...
| BE-008 Public consent/application/verification routes | ACTIVE (BE-008a + BE-008b-1 DONE; BE-008b-2 next) | GATE-03, BE-006, BE-007 | Split into child packets. **BE-008a** `GET /v1/public/consent-documents` + `consentRepository.findCurrentDocuments` DONE. **BE-008b-1** `src/crypto/` primitives (opaque token, keyed HMAC-SHA-256, AES-256-GCM envelope, email mask) + key config/`CryptoContext` DONE. Next: **BE-008b-2** `POST /v1/applications` submission + repository impls; **BE-008c** verify-email + delete `website/services/onboardingService.js`. Public route monolith `publicRoutes.js` -> BE-013 |
