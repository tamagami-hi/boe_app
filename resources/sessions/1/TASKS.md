...
| BE-008 Public consent/application/verification routes | ACTIVE (BE-008a DONE; BE-008b next) | GATE-03, BE-006, BE-007 | Split into child packets. **BE-008a** `GET /v1/public/consent-documents` + first repository impl (`consentRepository.findCurrentDocuments`) + route registrar, proven on PG (integration 16/16) DONE. Next: **BE-008b** `POST /v1/applications`, **BE-008c** verify-email + delete `website/services/onboardingService.js`. Public route monolith `publicRoutes.js` -> BE-013 |
