...
| RA-C Client financial routes + admin/login wiring | ACTIVE | RA-B, BE-021 | Multi-batch. **RA-C.1 DONE** admin web-auth [log](./logs/RA-C-1-admin-web-auth-wiring.md); **RA-C.2 DONE** client native-auth [log](./logs/RA-C-2-client-native-auth-wiring.md) — **login now works for both surfaces**. Remaining: admin applications queue wiring (`/v1/admin/applications*`, routes exist); then the large build — repositories+command services+routes for the `/v1/client/*` financial surface over the BE-021 schema (spec 03 §6/§7 money-math/locking/maker-checker, with integration tests) + client data screens |
