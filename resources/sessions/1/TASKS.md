...
| BE-007 Canonical identity/onboarding schema | ACTIVE (BE-007a/b/c/d/e/f DONE; BE-007g next) | BE-004, BE-005 | Split into child packets. **BE-007a** (`009` onboarding) + **BE-007b** (`010` identity) + **BE-007c** (`011` sessions) + **BE-007d** (`012` RBAC/audit/idempotency/rate-limit/legal-holds) + **BE-007e** (`013` outbox/email delivery) DONE; **BE-007f** full Kysely `Database` schema types + §7 repository interface contract (`src/db/types.ts`, `repositories.ts`, `limits.ts`) DONE, typed round-trip proven on PG (integration 14/14); next: bootstrap seed (BE-007g) |
| BE-008 Public consent/application/verification routes | BACKLOG | GATE-03, BE-006, BE-007 | Canonical Fastify handlers; delete onboarding service; public route monolith belongs to BE-013 |
