...
| BE-010 Activation and web/native auth routes | ACTIVE (native core landed; native-refresh + web + wiring + legacy deletion remaining) | GATE-03, CON-009, BE-006, BE-008, BE-009 | Accelerated single task (no sub-packets). Landed: session/credential/user/invite repositories, refresh-derivation + phone(mask) utils, `domain/auth/nativeAuth.ts` (activate/login/logout + bearer guard), `routes/nativeAuthRoutes.ts` (`/v1/activations/complete`, `/v1/auth/native/{login,logout}`); critical integration test (31/31). Remaining: `/v1/auth/native/refresh` rotation (30s grace + reuse revoke), web cookie/CSRF auth, production `server.ts` wiring, delete `security/auth.js`+`authService.js`+`authRoutes.js`. [log](./logs/BE-010-auth.md) |
