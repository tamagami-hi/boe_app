...
The backend controller is fully TypeScript (0 authored JS, permanently gated),
with the first-slice vertical, production server composition (PROD-001), and the
complete canonical later-domain schema (migrations 014-018) landed on branch
`ts-migration/backend` (PR #1). The active workstream is **backend finalization**
— completing the deferred first-slice items below — before any frontend work
begins. Full later-domain financial/catalog **routes** are large new feature
slices tracked under GATE-07/GATE-08, not part of finalization.
...
| RA-B Wire landing signup to the new backend | DONE | RA-B0 | Both landing signup surfaces (lead form + account form) wired to `POST /v1/applications` via a Next BFF (fetches consent versions + Idempotency-Key + E.164), adopting the application->verify->approval->activation model + consent acceptance. Landing build + tests green. [log](./logs/RA-B-landing-signup-wiring.md) |
| RA-C Client financial routes + admin/login wiring | ACTIVE | RA-B, BE-021 | Multi-batch. **RA-C.1** admin web-auth, **RA-C.2** client native-auth (login works for both surfaces), **RA-C.3** admin applications queue + review/decision wiring, **RA-C.4** client portfolio *read* slice (`GET /v1/client/{eligibility,holdings,orders}` native-auth over BE-021; derived eligibility spec 03 §2.3; portfolio/orders/eligibility client services wired; integration 84 -> 94), **RA-C.5** client order *write* path (`POST /v1/client/orders` create + `POST /v1/client/orders/:id/pay` beginPayment; eligibility re-derived under lock, published-fund + minimum guards, idempotency + audit + provider-call outbox; Order/Payment write repos; integration 94 -> 104), **RA-C.6** order booking money-math (`sendPaymentToProvider`/`confirmPayment`/`bookOrder` system commands; exact round-half-even `src/finance/money.ts`; booking appends execution+holding+lot+movement+notification; unit 313 -> 327, integration 104 -> 109), **RA-C.7** payment settlement worker (`settleMockPayment`/`advancePaymentToBooked` driver + `settleDuePayments` worker; `worker:payments` entrypoint; paid orders reach booked + holdings populate; unit 327 -> 329, integration 109 -> 113), **RA-C.8** env-driven payment provider + paid/failed confirmation checkpoint (provider + gateway creds + webhook secret from env; signed `POST /v1/provider-events/payment` drives confirm+book / fail, idempotent; `failPayment`; provider-aware worker; integration 113 -> 119), **RA-C.9** SIP slice (createSip/requestSipMandate/pause/resume/cancel + `POST /v1/client/sips*`; signed mandate webhook; installment scheduler `worker:sips`; SIP/Mandate repos; frontend SIP services; integration 119 -> 130) — all DONE (logs `RA-C-1..9`). **Remaining: rest of the write domain** — a real gateway outbound API client behind the `dispatchPayment` seam (creds already wired), then redemptions (spec 03 §5-§6 locking/maker-checker, with integration tests), then remaining client data screens (SIP list read, mandate-authorization UI step). Build slice-by-slice |
| RA-C.10 KYC email-OTP + frictionless eligibility | DONE | RA-C.9, BE-021, decisions 8-10 | Compliance write slice. Email-OTP KYC over `kyc_cases`: `POST /v1/client/kyc/{start,resend}` (ensure open case, hashed 6-digit code, resend cooldown, code emailed from the company mailbox post-commit) + `POST /v1/client/kyc/verify` (constant-time compare; 10-min expiry, 5 attempts; failed-attempt increment commits via an outcome type; -> `approved` with expiry). Eligibility simplified to `active + current approved KYC` (risk gate dropped; `deriveInvestingEligibility` + sip/createOrder + eligibility endpoint + unit tests updated; `risk_assessments` dormant). Transport-agnostic `EmailSender` port + `nodemailer@7` SMTP adapter (env `KYC_EMAIL_FROM` + `EMAIL_SMTP_*`) + log fallback. Migration `019` adds a **dedicated `kyc_verification_codes` table** (chosen over overloading `verification_tokens`, for OTP attempt semantics). Frontend `kycApi` (start/resend/verify; legacy fetchKycStatus/updateKycDepth preserved). Integration 130 -> 136 incl. end-to-end signup->activate->verify-email->eligible->invest; gates + guards green; `nodemailer@7` added. [log](./logs/RA-C-10-kyc-email-otp-and-eligibility.md) |
...
| BE-020 Backend zero-JS gate | DONE (backend authored JS/JSX = 0; permanent gate active) | BE-003..019 | `src/zero-legacy-js.guard.test.ts` scans `src/` + `scripts/` (excluding `dist/`, `node_modules/`) and asserts zero authored `.js/.jsx/.cjs/.mjs` AND zero legacy `#`-subpath alias imports in TypeScript. Runs under `npm run check`. Backend controller is now fully TypeScript: `find backend_controller/src backend_controller/scripts -type f \( -name '*.js' -o -name '*.jsx' \) \| wc -l` -> **0**. [log](./logs/BE-020-zero-js-gate.md) |
| PROD-001 Backend server composition wiring | DONE | BE-006, BE-008..016, BE-011, BE-012 | `src/runtime/composition.ts` `composeBackend(env)` builds pool/database/unit-of-work/crypto/access-token/breach-checker/certificate-fetcher/repositories and registers every canonical route group; `server.ts` composes + serves + drains; both smokes boot the full composed server. [log](./logs/PROD-001-server-composition.md) |
| BE-021.1 Later-domain schema increment 1 (compliance/catalog/platform) | DONE | BE-007, spec 03 §4 | Additive migrations `014_canonical_compliance.sql`/`015_canonical_catalog.sql`/`016_canonical_platform.sql` (16 tables); migration-apply integration test; no TS consumers yet. [log](./logs/BE-021-later-domain-schema-1.md) |
| BE-021.2 Later-domain schema increment 2 (investing/ownership + payments) + Kysely types | DONE | BE-021.1, spec 03 §4.3/§4.4 | Additive migrations `017_canonical_investing.sql`/`018_canonical_payments.sql` (12 tables) with composite ownership FKs, append-only booked evidence, paise `bigint`/`numeric(24,8)`; Kysely `Database` types + `Row<>` aliases for ALL later-domain tables (014-018); integration 69 -> 75. **Later-domain schema (spec 03 §4.1-§4.5) complete.** [log](./logs/BE-021-later-domain-schema-2.md) |

## Backend Finalization Packets

The backend first-slice + full later-domain schema are complete. These packets
finish the deferred first-slice items so the backend controller is production-
complete and self-consistent before frontend work. They are additive/hardening
(no authored JS may return; the zero-JS gate stays green).
