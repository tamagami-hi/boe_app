...
| RA-C Client financial routes + admin/login wiring | ACTIVE | RA-B, BE-021 | Multi-batch. **RA-C.1 DONE**: admin web-auth wiring (login/session/logout -> `/v1/auth/web/*` cookie+CSRF; reachability -> `/v1/health`) [log](./logs/RA-C-1-admin-web-auth-wiring.md). Remaining: client (native) auth wiring; admin applications queue (`/v1/admin/applications*`); then repositories+commands+routes for the `/v1/client/*` financial surface over the BE-021 schema (spec 03 §6/§7). Client + admin http mode work end-to-end for covered flows |
