...
| BE-009 Password/token/session security core | ACTIVE (BE-009a/b/c DONE; BE-009d next) | GATE-03, BE-007, CON-009 | Split into child packets. **009a** `passwordHasher.ts` (Argon2id) + deleted `security/passwords.js` (82->81). **009b** `breachCheck.ts` (HIBP). **009c** `accessToken.ts` (ES256 jose sign/verify, kid selection, pinned iss/aud/typ/skew, 10-min TTL). Next: **009d** refresh/CSRF rotation; then delete `security/{auth,tokens}.js` (81->79) |
