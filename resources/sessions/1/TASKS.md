...
| BE-009 Password/token/session security core | ACTIVE (BE-009a/b DONE; BE-009c next) | GATE-03, BE-007, CON-009 | Split into child packets. **BE-009a** `src/auth/passwordHasher.ts` (Argon2id) + pinned deps; deleted `security/passwords.js` (82 -> 81). **BE-009b** `src/auth/breachCheck.ts` (HIBP k-anonymity, injectable fetch, fail-closed, bypass test/dev-only). Next: **009c** ES256 access tokens (jose), **009d** refresh/CSRF rotation; then delete `security/{auth,tokens}.js` |
