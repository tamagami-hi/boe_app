...
| BE-007 Canonical identity/onboarding schema | ACTIVE (BE-007a,BE-007b DONE; BE-007c next) | BE-004, BE-005 | Split into child packets. **BE-007a** (onboarding schema `009`) + **BE-007b** (identity tables `010`: users/credentials/reviews/invites + verification FK) DONE, applied+asserted on PG (integration 10/10); next: `auth_sessions`/`auth_refresh_tokens`, RBAC/audit/outbox/email, repositories, bootstrap seed |
