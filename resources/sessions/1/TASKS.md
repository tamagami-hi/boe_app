...
| RA-C Client financial routes + admin/login wiring | BACKLOG | RA-B, BE-021 | Repositories+commands+routes for the `/v1/client/*` surface the frontend calls, over the BE-021 schema (spec 03 §6/§7); plus wiring web/native login (`/v1/auth/web/*`) and the admin identity queue (`/v1/admin/*`). Client + admin http mode work end-to-end for covered flows |
