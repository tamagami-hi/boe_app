...
|---|---|---|
| HTTP transport | Consolidated by injection | Shared app-config operations receive the client/admin canonical `apiRequest`; the shared package does not duplicate auth/CSRF transport. |
...
| Payment state mapping | Consolidated on 2026-08-28 | `frontend_stack/packages/shared/src/paymentStates.js` is the canonical client/admin presentation vocabulary; backend projections remain the business authority. |
| Form primitives | Consolidated on 2026-08-28 | Admin `FormField` delegates to the tested shared primitive and supplies styling hooks only. |
...
|---|---|---|
| App config | Simplified on 2026-08-28 | Presentation defaults remain local; business fixtures, embedded product/research catalogues, and stale AutoPay disclosures were removed. Remote reads use the canonical request boundary. |
...
| Fixture mode | Removed on 2026-08-28 | Production service modules are HTTP-backed and fail explicitly; deleted fixture records remain recoverable from Git. |
...
- Proven-unreachable admin aliases; the active legacy-named route wrapper was
  renamed to `pages/adminRoutes.jsx` on 2026-08-28.
...
- Product/accounting contract for refund initiation. Existing rows can be
  dispatched/reconciled, but no creation route exists and refund completion has
  no canonical client-ledger reversal transaction.
- Deployed contents and removal safety of persistent `rate_limit_windows` and
  `finance_policy_versions`; their unused runtime abstractions are not a reason
  to perform an unverified destructive migration.
...
Verified 2026-08-28 implementation baseline:

- Backend `npm run check`: 73 files and 666 unit tests passed; statement/line
  coverage 81.43%, branch coverage 80.04%; typecheck, lint, build, and source/dist
  smoke checks passed.
- Backend integration behavior: 19 files and 208 tests passed. The standalone
  integration command exits non-zero only because its isolated subset reports
  78.39% coverage against the repository-wide 80% threshold; the canonical full
  unit check meets the threshold.
- Frontend: 69 files and 900 tests passed; production build and 11-chunk bundle
  boot check passed.

Do not weaken assertions or coverage thresholds during later simplification.
...
### Stage 2 — Contract authority (separate active workstream)
...
### Stage 3 — Close or remove redemption (product decision required)
...
### Stage 4 — Consolidate frontend transport/state (completed for current scope)
...
### Stage 5 — Remove compatibility and non-product baggage (completed for active fixture/wrapper scope)
