...
| Debug logging is needed for physical-device payment diagnosis but must never reach production. | Development/local targets build debuggable APKs by default. Production alone builds `assembleRelease`, requires configured release signing, and fails closed unless APK inspection proves `debuggable=false`. Development may use the release certificate solely to preserve the installed app identity and data across test updates. |
| A debuggable development APK could cause PhonePe to emit payment intent/token diagnostics. | APK debuggability and provider logging are separate controls. PhonePe SDK initialization uses `enableLogging=false` for every target; use the bounded, redacting Android capture utility for diagnostics. Reusing the current release certificate for the distinct `.dev` package remains an explicit UAT continuity tradeoff until a dedicated stable UAT key and corresponding PhonePe registration are coordinated. |
| PhonePe checkout opens but the provider rejects the payment with a generic processing error. | Treat this as a provider/configuration release blocker, not an application success or a reason to bypass reconciliation. The signed failure event and later status lookup converged the canonical records to `PROVIDER_DECLINED`. Verify PhonePe merchant environment entitlement and registration of the exact `com.beonedge.app.dev` application ID/signing certificate before another low-value test. |
| Native one-time checkout requires a PhonePe-onboarded Android package/app URL, but DEV uses `com.beonedge.app.dev`. | Use hosted Standard Checkout for one-time and manual-installment payments. The two successful INR 2 transactions and `v0.10.7` prove this route. Keep native SDK use limited to AutoPay mandate authorization, which still requires PhonePe to register the exact DEV package and signing certificate before AutoPay UAT. |
| A compromised or malformed provider response could redirect a client to an attacker-controlled HTTPS site. | The backend accepts checkout URLs only from exact configured HTTPS origins, and the frontend rejects unsafe schemes before same-WebView navigation. Current production allowlist is `https://mercury.phonepe.com,https://mercury-t2.phonepe.com`; a newly observed legitimate PhonePe origin must be verified and explicitly added. |
