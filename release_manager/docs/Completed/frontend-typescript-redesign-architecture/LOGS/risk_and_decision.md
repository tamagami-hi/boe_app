...
**Client presentation is a mapping layer, not a rename of domain values.**

The client UI exposed raw UUIDs, provider slugs, provider failure codes, snake_case event kinds,
record versions and server error messages. The obvious shortcut is to change the values. That is
wrong: the API contract, the payment state machine, the mandate lifecycle and the admin console
all depend on those values being exactly what they are.

So every one of them is translated at the edge instead, in seven modules under `src/domain/`:
`failure.ts`, `paymentReason.ts`, `provider.ts`, `notifications.ts`, `clientStatus.ts`,
`plural.ts`, `reference.ts`.

Three rules fall out of this and constrain later work:

1. **An unmapped internal value renders nothing.** `paymentFailureReason`,
   `paymentPartnerName` and `notificationCategory` all return `null` for input they do not
   recognise, and every call site omits the row or line. They never fall back to printing the
   token. A new provider code or notification kind therefore degrades to silence, not to
   leaked internals — but it also means **adding a backend value requires adding its mapping**
   or the client will show nothing where something is expected.

2. **`error.message` is never client copy.** `describeClientFailure(error, context)` is the only
   sanctioned path from an error to a rendered string. It has a total fallback, so it cannot
   return server text. `failureDiagnostics()` exists for logging and must not be rendered. Any
   new mutation needs a `FailureContext`, not an inline message.

3. **Client labels and admin labels are separate functions.** `sipState`, `mandateState`,
   `mandateSetupState` and `emailVerificationState` in `domain/status.ts` are shared with the
   admin console and keep their precise internal vocabulary — an operator needs `Dispatching`.
   The client equivalents live in `clientStatus.ts`. Do not "fix" the admin labels for the
   client's benefit; add to `clientStatus.ts`. `clientInvestmentStatus` is the exception and was
   relabelled in place, because both of its consumers are client screens and its name already
   declares its audience.

**Consequence for the mobile heading rule.** `ClientFrame` reveals the route title only past
56 px of scroll, which makes `PageHeader` the single primary heading at rest. A screen that
renders no `PageHeader` will therefore show no title at all until scrolled. Every client screen
currently has one; a new screen must too.

**What this does not cover.** The transactional back confirmation is still a native
`window.confirm`, because `NativeBackCoordinator`'s hardware-back handler is synchronous and
