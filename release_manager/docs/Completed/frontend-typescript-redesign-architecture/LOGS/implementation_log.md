...
## Entry 042 — client production UI/content pass: 169 audit findings dispositioned, presentation layer introduced · 2026-09-07

**Decision:** [D-072](risk_and_decision.md#d-072)

Two phases in one session. First a read-only extraction of the whole client frontend into
`release_manager/audit_pages/client/` — 25 routes, 118 screenshots, 92 machine-readable capture
records, 169 findings. Then the production implementation against those findings.

### What the extraction found, measured from rendered text rather than source

Scanning the `innerText` of all 92 captures, not grepping the codebase:

| | pages |
|---|---|
| raw UUIDs visible to the client | 1 — `011_payment_status` showed two, labelled `Payment reference` and `Order` |
| raw snake_case tokens visible | 1 — `015_notifications` printed `fund_receipt_acknowledged` and `app_update_available` |
| architecture vocabulary visible | 20 of 25 pages, 10 distinct terms |
| screens rendering `error.message` as client copy | 4 |

The payment screen was the worst case: two internal UUIDs and the raw provider slug `phonepe`
were the most prominent content on it, and the **fund name was absent entirely**. The
provider-facing reference that actually reaches the bank statement is `boe_<32 hex>`
(`backend_controller/src/domain/payments/merchantIds.ts`) — neither of the two UUIDs the client
was shown.

Two real defects, not just wording:

1. **`/blocked` told an active account it was suspended.** The screen branched on
   `closed` versus everything else, so any account reaching the URL got "Account suspended".
   The audit screenshots are of an active account making exactly that false claim.
2. **`Any day from 1 to 28`** on the SIP screen, above a control offering six days.

### The presentation layer

Seven new modules under `src/domain/`. The rule they enforce: internal values stay internal, and
nothing raw reaches a client screen.

- `failure.ts` — `describeClientFailure(error, context)` across 13 contexts, covering transport
  errors, every contract error code and unknown errors. It cannot return `error.message`. The
  four `describeFailure` implementations in `LumpsumInvestScreen`, `SipStartScreen`,
  `SipDetailScreen` and `LoginScreen`'s `messageFor` are deleted and route through it.
  `failureDiagnostics()` keeps the raw detail available for logging without rendering it.
- `paymentReason.ts`, `provider.ts`, `notifications.ts` — unmapped input returns `null` so the
  row or line is **omitted** rather than printing a token. That is the empty-value policy: an
  absent value is not an em dash.
- `clientStatus.ts` — client SIP, AutoPay and email-verification presentations, each carrying a
  `detail` sentence saying what to do.
- `plural.ts`, `reference.ts`.

**`sipState`, `mandateState`, `mandateSetupState` and `emailVerificationState` in
`domain/status.ts` were deliberately left alone.** They are shared with the admin console, where
`Dispatching` and `Awaiting authorisation` are the correct words for an operator. Relabelling
them for the client would have degraded admin. `clientInvestmentStatus` *was* relabelled in
place because both consumers are client screens.

### Shared foundation first

Ten of the findings were single-source: `AuthLayout`'s three markers and footnote appeared on
three screens; `ErrorState` supplied the entire error vocabulary of the app; `LoadMore`,
`ConfirmDialog` and `AsyncBoundary` were each one string away from correct on every screen that
used them.

The mobile duplicate-heading problem was solved as a rule rather than per page: `ClientFrame`
now reveals the route title in the top bar only past 56 px of scroll, resetting on route change,
so `PageHeader` is the single primary heading at rest on both viewports. The `useState` and two
`useEffect` calls sit above the existing early return so hook order stays stable for public
routes.

The transactional back confirmation was scoped: it now fires only on `/invest/lumpsum` and
`/invest/sip`, with route-specific wording. The payment-status screen holds no unsaved entry and
no longer claims data will be lost.

### Result

```text
                                        before   after
pages showing a raw UUID                     1       0
pages showing a snake_case token             1       0
architecture terms visible                  10       0
screens rendering error.message              4       0
horizontal overflow at 390x844               0       0
```

169 findings: **132 FIXED, 19 ACCEPTED with a recorded reason, 14 REQUIRES PRODUCT DECISION,
4 REQUIRES LEGAL CONTENT.** No finding was closed by deleting a feature.

No legal copy was fabricated. The investor charter and grievance policy are unpublished on this
stack; the fix was to stop the UI contradicting itself — the raw `Version 1` line is gone and
the `Updated` line is now suppressed unless the document actually has renderable content, so the
app no longer claims a document was updated while telling the client it does not exist.

### Verified

- **TESTED** `npm run typecheck`, `npx eslint src` (0 findings), `npx vitest run` (21 files,
  **199 tests**, all pass), `npm run build`, `check-bundle-boots`, `check-android-dist`.
- **TESTED** browser pass: local production `dist/` served at the deployed origin through
  Playwright route interception so API calls hit the real dev backend. 22 routes at 1440×900 and
  390×844, full page, seed QA account. No horizontal overflow. Only console errors are the
  expected 401 from the anonymous session-restore probe, same as baseline.
- **TESTED** APK pass: rebuilt via `./emu/boe_update.sh --local --client --install` and driven
  over the WebView CDP socket on the running emulator, verified account with real holdings and
  25+ payments. Covered 007, 008, 011, 013 — which the browser cannot reach because of the
  eligibility guard — plus populated variants of eight more screens.
- **TESTED** the `/blocked` fix behaviourally: `/blocked` on an active account now lands on
  `/dashboard` instead of rendering "Account suspended".
- **STATIC** the suspended and closed branches of `/blocked`, since no such account exists on
  this stack.

### Not verified

- **No payment was initiated and no SIP was created, paused, resumed or cancelled.** The new
  failure copy for those paths is source-verified only; it has not been rendered from a real
  provider refusal.
- The device lock screen, the app-update surfaces and the four SIP confirmation dialogs were not
  opened — each needs a device PIN, a changed update feed, or acting on the maintainer's live
  plan.
- AutoPay is still blocked on PhonePe merchant provisioning, so the AutoPay copy is visible only
  on a plan already sitting in `Awaiting AutoPay`.
- No test was added. Per `README.md` §4 these are copy and presentation changes; the one
  behavioural change is `/blocked` routing, and `routeIntegrity.test.ts` already covers the
  manifest and guard wiring it depends on. If that routing is considered security-relevant it
  deserves a targeted test, which is a deliberate judgement call left open here.
