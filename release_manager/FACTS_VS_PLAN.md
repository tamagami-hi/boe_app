...
| Separate admin + user frontend apps | **One** Vite shell `frontend_stack_ts`, switched by build-time `VITE_BEO_APP_TARGET` (`client` → user, `admin` → admin). One source tree, two shells under `src/shells/` | Build `frontend_stack_ts` **twice** with different args |
| App and admin images are built separately | `frontend_stack_ts/Dockerfile` accepts `VITE_BEO_APP_TARGET` and serves each static bundle with digest-pinned, unprivileged nginx on container port **8080**. Built from the repository root so `packages/contracts` is in the context | Implemented by the release exporter; host ports remain stack-specific |
